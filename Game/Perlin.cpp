...
// Initialize with const permutation vector
Perlin::Perlin() {
...
double Perlin::noise(double x, double y, double z) {
	...
	// Find relative x, y, and z of the point in unit cube
	x -= floor(x);
	y -= floor(y);
	z -= floor(z);
	...
	(zFade,
		...
		(yFade,
			lerp
			(xFade, grad(p[AA], x, y, z),
				grad(p[BA], x - 1, y, z)
			),
			lerp
			(xFade, grad(p[AB], x, y - 1, z),
				grad(p[BB], x - 1, y - 1, z)
			)
		),
		lerp
...
double Perlin::octaveNoise(double x, double y, double z, int octaves, double persistence)
{
	double total = 0;
	double frequency = 4;
	double amplitude = 1;
